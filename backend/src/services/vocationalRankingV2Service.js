...
const SIGNAL_AFFINITY_ROUTES = Object.freeze({
  interest:Object.freeze({subjects:1,activities:.9,workStyles:.25,contexts:.2}),
  ability:Object.freeze({abilities:1}),
  preference:Object.freeze({workStyles:1,contexts:.9,activities:.35}),
  restriction:Object.freeze({contexts:1})
});
...
export function extractAcademicStageV2(text) {
  ...
  const stages=[
    ...
    ["tsu",/(?:tsu|tecnico superior universitario)/],["licenciatura",/(?:licenciatura|universidad|ingenieria)/],
  ...
  const current=/\b(?:estoy (?:en|cursando|estudiando)|curso|voy en|actualmente estudio)\b/.test(value);
...
export function getSignalProgramAffinity(signal,programProfile){
  const routes=SIGNAL_AFFINITY_ROUTES[signal?.dimension]||{},matches=[];
  for(const [profileDimension,coefficient] of Object.entries(routes)){
    const traitWeight=programProfile?.traitWeights?.[profileDimension]?.[signal.conceptId]||0;
    if(traitWeight>0)matches.push({profileDimension,coefficient,traitWeight,weightedAffinity:traitWeight*coefficient});
  }
  if(signal?.dimension==="restriction"){
    const restriction=programProfile?.contextRestrictions?.[signal.conceptId];
    if(restriction?.affinity>0)matches.push({profileDimension:"contextRestrictions",coefficient:1,traitWeight:restriction.affinity,weightedAffinity:restriction.affinity});
  }
  const affinity=matches.length?Math.max(...matches.map(match=>match.weightedAffinity)):0;
  return {affinity:Math.round(affinity*1000)/1000,sourceDimensions:matches.filter(match=>Math.abs(match.weightedAffinity-affinity)<1e-9).map(match=>match.profileDimension),matches};
}
...
export function rankFullVocationalCatalog({ vocationalProfile, currentRevision, candidateSource="profile_inference", explicitProgramId=null }={}) {
  ...
  const ordered=Object.keys(profiles).map(id=>{const profile=profiles[id], breakdown=[]; let score=0,pos=0,neg=0,blocked=false;
    ...
    for(const s of p.signals){const restriction=profile.contextRestrictions?.[s.conceptId],affinityResult=getSignalProgramAffinity(s,profile),affinity=affinityResult.affinity;if(!affinity)continue;const key=`${s.dimension}${s.polarity[0].toUpperCase()+s.polarity.slice(1)}`;const base=WEIGHTS[key]||0,value=base*(s.intensity/3)*recency(revision-s.updatedRevision)*affinity;score+=value;(value>=0?pos++:neg++);breakdown.push({code:key,conceptId:s.conceptId,value:Math.round(value*100)/100,affinity,sourceDimensions:affinityResult.sourceDimensions});if(s.dimension==="restriction"&&s.polarity==="negative"&&restriction?.hardRestrictionEligible&&restriction.affinity>=1)blocked=true;}
...
export { SIGNAL_AFFINITY_ROUTES, WEIGHTS as VOCATIONAL_V2_WEIGHTS };
